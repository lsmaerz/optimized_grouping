...
public class StandardImproved {
	 ...
	 */
	public static void execute(WritableSheet sheet, int timeLimit, double gap, String address, boolean moreInfo, int pos) {
		// Parameters
		double[][] Q_hat;
		double[][] M;
		double[][] Size;
		...
		try {
			...
			model.set(GRB.StringAttr.ModelName, "Standard Improved");
			...
			for (int i = 0; i < S; i++) {
				for (int j = i + 1; j < S; j++) {
					x[i][j] = model.addVar(0, 1, (1/(double)S)*(Q_bin[i][j] + Q_bin[j][i]), GRB.BINARY, "x(" + i + ", " + j + ")");
				}
			}
			...
			for (int i = 0; i < S; i++) {
				for (int j = i + 1; j < S; j++) {
					for (int k = j + 1; k < S; k++) {
						...
						model.addConstr(expr, GRB.GREATER_EQUAL, -1, "Transitivity I/III");

						expr = new GRBLinExpr();
						expr.addTerm(1, x[i][j]);
						expr.addTerm(-1, x[i][k]);
						expr.addTerm(-1, x[j][k]);
						model.addConstr(expr, GRB.GREATER_EQUAL, -1, "Transitivity II/III");

						expr = new GRBLinExpr();
						expr.addTerm(1, x[j][k]);
						expr.addTerm(-1, x[i][k]);
						expr.addTerm(-1, x[i][j]);
						model.addConstr(expr, GRB.GREATER_EQUAL, -1, "Transitivity III/III");
					}
				}
			}
